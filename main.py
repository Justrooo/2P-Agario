...
while texture == texture2:
    texture2 = pygame.image.load(choice(player.player_textures))
    pr2.image = texture2
...
# Right panel
panel_right = ui_scripts.ui_panel.Panel("Images/UI/box.png", w-500, 600)
# Group
...
p1text = ui_scripts.ui_text.Text("Fonts/button_font.ttf", 78, pr.nick, panel_left.rect.centerx,
                                 panel_left.rect.centery)
p2text = ui_scripts.ui_text.Text("Fonts/button_font.ttf", 78, pr2.nick, 0, 0)
# Fonts & Texts ------------------------------------------------------------
# InputBox -----------------------------------------------------------------
...
while True:
    ...
    if menu_state == 0:
        ...
        screen.blit(p1text.update_text(), p1text.rect.center)  # P1 nick update
        screen.blit(p2text.update_text(), (100, 100))  # P2 nick update
        ...
        if pr.rect.right >= w: pr.rect.left = pr.rect.x - 10
        if pr.rect.top <= 0: pr.rect.top = pr.rect.y + 10
        if pr.rect.bottom >= h-50: pr.rect.top = pr.rect.y - 10
